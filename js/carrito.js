...
function cargarProductos() {
    ...
    document.getElementById("total-productos").innerText = `Total de productos: ${totalProductos}`;
       //Domicilio 
...
function completarCompra() {
    ...
    //límite de cantidad y presupuesto 
    const cantidadMaxima = parseInt(localStorage.getItem("cantidadMaxima"), 10);
    const presupuestoMaximo = parseInt(localStorage.getItem("presupuestoMaximo"), 10);
    ...
    setTimeout(() => {
        if (window.totalProductosSeleccionados > cantidadMaxima) {
            alert("No se pudo completar la compra. La cantidad de productos seleccionados excede el límite permitido.");
            return;
        }

        if (window.totalCompraActual > presupuestoMaximo) {
            alert("No se pudo completar la compra. El valor total de la compra excede el presupuesto máximo.");
            return;
        }

        if (numeroTarjeta.length === 16 && fechaExpiracion.length === 5 && cvv.length === 3) {
...
// Solo ingresaran digitos en el numero de tajeta
document.getElementById("numero-tarjeta").addEventListener("input", function (e) {
    e.target.value = e.target.value.replace(/\D/g, ""); 
});

// Solo ingresaran digitos en forma de fecha en la fecha de expiracion
...
// Solo ingresaran digitos en el codigo de seguridad
